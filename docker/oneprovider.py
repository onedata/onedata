...
# Username to be used in Basic auth with the root password
ROOT_USERNAME = "root"
ROOT_PASSWORD_VARIABLE = 'ONEPANEL_ROOT_PASSWORD'
...
# Returns root password provided in environment variable
# or None if it is not set
def get_root_password():
    return os.environ.get(ROOT_PASSWORD_VARIABLE)
...
def set_root_password(password):
    ...
    r = do_request(password, requests.put,
                   'https://127.0.0.1:9443/api/v3/onepanel/root_password',
                   headers={'content-type': 'application/json'},
                   data=json.dumps({'password': password}),
                   verify=False)
    if r.status_code != 204:
        raise ValueError('Could not set Onepanel root password: ' + r.text)


def do_request(root_password, request, *args, **kwargs):
    r = request(*args, auth=(ROOT_USERNAME, root_password), **kwargs)
    if r.status_code != 401 and r.status_code != 403:
        return r

    raise AuthenticationException('Authorization error.\n'
        'Please ensure that valid root password is present\n'
        'in the ' + ROOT_PASSWORD_VARIABLE + ' environment variable')
...
def configure(config):
    ...
    try:
        ...
        # root password setup failure indicates existing deployment
        return False

    r = do_request(root_password, requests.post,
    ...
    while status == 'running':
        r = do_request(root_password, requests.get,
