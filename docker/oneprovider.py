...
def set_autogenerate_cert(file_path, autogenerate_cert, domain):
    flag = 'true' if autogenerate_cert == 'true' else 'false'
    replace(file_path, r'{generate_web_cert_if_absent, .*}',
            '{{generate_web_cert_if_absent, {0}}}'.format(flag))
...
if __name__ == '__main__':
    try:
        ...
        autogenerate_cert = os.environ.get('ONEPANEL_AUTOGENERATE_CERT')
        if autogenerate_cert:
            domain = os.environ.get('ONEPANEL_AUTOGENERATED_CERT_DOMAIN')
            set_autogenerate_cert(
                '/etc/op_panel/app.config', autogenerate_cert, domain)
