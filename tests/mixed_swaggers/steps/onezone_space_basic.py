...
@when(parsers.parse('{username} creates space "{space_name}" in "{zone_name}" Onezone service'))
def create_spaces(username, space_name, zone_name, hosts, users):
    Conf_OZ().verify_ssl = False
    Conf_OZ().username = username
    Conf_OZ().password = users[username].password

    user_client = ApiClient_OZ(host="https://{}:{}{}".format(
