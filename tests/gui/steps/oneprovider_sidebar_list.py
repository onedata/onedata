...
def _get_items_from_sidebar_list(driver, ul_type):
    items = driver.find_elements_by_css_selector('ul.{ul_type}-list '
...
def _select_items_from_sidebar_list(driver, browser_id, tmp_memory,
    ...
    for item_name in list_parser(items_names):
        ...
        if item and 'active' not in item.get_attribute('class'):
            ...
            raise ValueError('no {} named {} found'.format(items_type[:-1],
                                                           item_name))
...
def is_present_in_sidebar_list(selenium, browser_id, item_list,
                               item_type, tmp_memory):
    driver = select_browser(selenium, browser_id)
    _in_sidebar_list = lambda d, n, t: not _not_in_sidebar_list(d, n, t)
    Wait(driver, MAX_REFRESH_COUNT * WAIT_BACKEND).until_not(
        lambda d: refresh_and_call(d, lambda d, n, t: not _not_in_sidebar_list(d, n, t),
