Feature: Oneprovider Group functionality
  ...
  Scenario: User creates new group (presses ENTER after entering group name)
    ...
    Then user of browser sees that new item has appeared on groups sidebar list
  ...
  Scenario: User fails to view group after leaving it
    ...
    Then user of browser sees an error notify with text matching to: Cannot load requested resource: undefined
