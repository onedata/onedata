...
fsonedatafs_py2_package = [path for path in packages
                         if path.startswith('onedata{release}-python2-fs-onedatafs'.format(release='1802'))][0]

fsonedatafs_py3_package = [path for path in packages
                         if path.startswith('onedata{release}-python3-fs-onedatafs'.format(release='1802'))][0]
...
check_call(['scl', 'enable', 'onedata{release}'.format(release='1802'), 'PYTHONPATH="${ONEDATA_PYTHON2_PATH}" python2 -c "from fs.onedatafs import OnedataFS"'])

# validate onedatafs Python3 package installation
check_call(['scl', 'enable', 'onedata{release}'.format(release='1802'), 'PYTHONPATH="${ONEDATA_PYTHON3_PATH}" python3 -c "from fs.onedatafs import OnedataFS"'])
