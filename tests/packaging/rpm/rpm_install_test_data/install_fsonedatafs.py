...
onedatafs_py2_package = [path for path in packages
                         if path.startswith('onedata{release}-python2-onedatafs'.format(release='2102'))][0]
...
fsonedatafs_py2_package = [path for path in packages
                         if path.startswith('onedata{release}-python2-fs-onedatafs'.format(release='2102'))][0]
...
check_call(['scl', 'enable', 'onedata{release}'.format(release='2102'), 'PYTHONPATH="${ONEDATA_PYTHON2_PATH}" python2 -c "from fs.onedatafs import OnedataFS"'])
