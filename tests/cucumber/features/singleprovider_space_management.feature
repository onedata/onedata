Feature: Space management with single provider
  ...
  Scenario: Default space without support
    ...
    Then u1 can't list u1's space on client1
  ...
  Scenario: Invite user to default space
    ...
    When u2 doesn't see [u1's space] in . on client1
    When u1 invites u2 to space u1's space
    And u2 joins space u1's space
    And u2 sees [u1's space] in . on client1
    And u2 can't list s1 on client1
    And u1 asks for support of space [u1's space]
    And [u1's space] is supported for u1 by p1 with 1 MB
    And u2 can list u1's space on client1
    And u2 creates regular files [u1's space/file1] on client1
    Then u2 sees [file1] in u1's space on client1
    And u2 writes "TEST TEXT ONEDATA" to u1's space/file1 on client1
    Then u2 reads "TEST TEXT ONEDATA" from u1's space/file1 on client1

  Scenario: Invite user to non-default space
    Given u2 starts oneclient in /home/u2/onedata using token on client1
    When u2 doesn't see [s1] in . on client1
    When u1 creates spaces [s1]
    And u1 invites u2 to space s1
    And u2 joins space s1
    And u2 sees [s1] in . on client1
    And u2 can't list s1 on client1
    And u1 asks for support of space [s1]
    And [s1] is supported for u1 by p1 with 1 MB
    And u2 can list s1 on client1
    And u2 creates regular files [s1/file1] on client1
    Then u2 sees [file1] in s1 on client1
    And u2 writes "TEST TEXT ONEDATA" to s1/file1 on client1
    Then u2 reads "TEST TEXT ONEDATA" from s1/file1 on client1
