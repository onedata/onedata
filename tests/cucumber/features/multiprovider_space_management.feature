Feature: Space management with multiple providers
  ...
  Scenario: Default space with support
    ...
    When u2 sees [u2's space] in spaces on client2
    And  u1 can't list spaces/u1's space on client1
    And  u2 can't list spaces/u2's space on client2
    ...
    And  u2 can list spaces/u2's space on client2
    And u1 creates regular files [spaces/u1's space/file1] on client1
    And u2 creates regular files [spaces/u2's space/file1] on client2
    Then u1 sees [file1] in spaces/u1's space on client1
    Then u2 sees [file1] in spaces/u2's space on client2
    And u1 writes "TEST TEXT ONEDATA" to spaces/u1's space/file1 on client1
    And u2 writes "TEST TEXT ONEDATA" to spaces/u2's space/file1 on client2
    Then u1 reads "TEST TEXT ONEDATA" from spaces/u1's space/file1 on client1
    Then u2 reads "TEST TEXT ONEDATA" from spaces/u2's space/file1 on client2
  ...
  Scenario: Default space of other user supported by other provider
    ...
    And  u2 can list spaces/u2's space on client2
    And u1 creates regular files [spaces/u1's space/file1] on client1
    Then u2 sees [file1] in spaces/u1's space on client2
    And u1 writes "TEST TEXT ONEDATA" to spaces/u1's space/file1 on client1
    Then u2 reads "TEST TEXT ONEDATA" from spaces/u1's space/file1 on client2
    And u2 creates regular files [spaces/u1's space/file2] on client2
    Then u1 sees [file2] in spaces/u1's space on client1
    And u2 writes "TEST TEXT ONEDATA" to spaces/u1's space/file1 on client2
    Then u1 reads "TEST TEXT ONEDATA" from spaces/u1's space/file1 on client1
  ...
  Scenario: Non-default space of other user supported by other provider
    ...
    And u2 doesn't see [s1] in spaces on client2
    ...
    And  u2 can list spaces/s1 on client2
    And u1 creates regular files [spaces/s1/file1] on client1
    Then u2 sees [file1] in spaces/s1 on client2
    And u1 writes "TEST TEXT ONEDATA" to spaces/s1/file1 on client1
    Then u2 reads "TEST TEXT ONEDATA" from spaces/s1/file1 on client2
    And u2 creates regular files [spaces/s1/file2] on client2
    Then u1 sees [file2] in spaces/s1 on client1
    And u2 writes "TEST TEXT ONEDATA" to spaces/s1/file1 on client2
  ...
  Scenario: Remove user from space
    ...
    Then u2 doesn't see [s1] in spaces on client2
  ...
  Scenario: Delete supported default space
    ...
    And [u1's space] is supported for u1 by p1 with 1 MB
    When u2 can list spaces/u1's space on client2
    And u2 deletes space u1's space
    Then u2 can't list spaces/u1's space on client2
